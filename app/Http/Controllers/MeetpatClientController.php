    ...
    public function get_saved_files(Request $request)
    {
        ...
        foreach($files_array as $key=>$file)
        {
            ...
            if(env('APP_ENV') == 'production') {
                ...
                {
                    $files_array->items()[$key]["download"] = \Storage::disk('s3')->temporaryUrl('client/saved-audiences/user_id_' . $request->user_id . '/' . $files_array[$key]["file_unique_name"] . '.csv', now()->addMinutes(15),
