    ...
    public function get_saved_files(Request $request)
    {
        ...
        foreach($files_array as $key=>$file)
        {
            if(env('APP_ENV') == 'production') {
                ...
                {
                    $files_array->items()[$key]["download"] = \Storage::disk('s3')->temporaryUrl('client/saved-audiences/user_id_' . $request->user_id . '/' . $files_array[$key]["file_unique_name"] . '.xlsx', now()->addMinutes(5),
                     ['Content-Type' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                      'ResponseContentType' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                      'ResponseContentDisposition' => 'attachment; filename=' . $files_array->items()[$key]["file_name"] . ".xlsx"]);
                    $files_array->items()[$key]["size"] = round(\Storage::disk('s3')->size('client/saved-audiences/user_id_' . $request->user_id . '/' . $files_array[$key]["file_unique_name"] . '.xlsx') / 1024 / 1024, 2) . "MB";
                } else {
                    $files_array->items()[$key]["download"] = "/404";
                    $files_array->items()[$key]["size"] = "N\A";
                }
                ...
                if(\Storage::disk('local')->exists('client/saved-audiences/' . 'user_id_' . $request->user_id . '/' . $files_array[$key]["file_unique_name"] . '.xlsx'))
                {
                    $files_array->items()[$key]["download"] = "/404";
                    $files_array->items()[$key]["size"] = round(\Storage::disk('local')->size('client/saved-audiences/user_id_' . $request->user_id . '/' . $files_array[$key]["file_unique_name"] . '.xlsx') / 1024 / 1024, 2) . "MB";
                } else {
                    $files_array->items()[$key]["download"] = "/404";
                    $files_array->items()[$key]["size"] = "N\A";
                }
