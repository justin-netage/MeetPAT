        ...
        if($request->searchPhrase)
        {
            ...
            foreach($files_array as $key=>$file)
            {
                ...
                if(env('APP_ENV') == 'production') {
                    ...
                    {
                        $files_array->items()[$key]["download"] = \Storage::disk('s3')->temporaryUrl('client/client-records/' . 'user_id_' . $request->user_id . '/' . $files_array[$key]["file_unique_name"] . '.csv', now()->addMinutes(5), ['Content-Type' => 'text/csv', 'ResponseContentType' => 'text/csv', 'ContentDisposition' => 'attachment; filename=' . explode(" - ", $files_array->items()[$key]["audience_name"])[0] . ".csv"]);
