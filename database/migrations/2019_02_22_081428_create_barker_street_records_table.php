...
class CreateBarkerStreetRecordsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('barker_street_records', function (Blueprint $table) {
            ...
            $table->enum('AgeGroup', ['01', '02', '03', '04', '05', '06', '07', '08'])->nullable();
            $table->enum('Gender', ['M', 'F'])->nullable();
            $table->enum('PopulationGroup', ['B', 'W', 'C', 'A', 'U'])->nullable();
            $table->enum('DeceasedStatus', ['true', 'false'])->nullable();
            $table->enum('MaritalStatus', ['true', 'false'])->nullable();
            $table->enum('DirectorshipStatus', ['true', 'false'])->nullable();
            $table->enum('HomeOwnerShipStatus', ['true', 'false'])->nullable();
            $table->integer('income')->nullable();
            $table->enum('incomeBucket', ['R0 - R2 500', 'R2 500 - R5 000', 'R5 000 - R10 000', 'R10 000 - R20 000', 'R20 000 - R30 000', 'R30 000 - R40 000', 'R40 000 +'])->nullable();
            $table->enum('LSMGroup', ['LSM00', 'LSM01', 'LSM02', 'LSM03', 'LSM04', 'LSM05', 'LSM06', 'LSM07', 'LSM08', 'LSM09', 'LSM10'])->nullable();
            $table->enum('CreditRiskCategory', ['VERY_LOW', 'LOW', 'MEDIUM', 'HIGH', 'VERY_HIGH'])->nullable();
            $table->enum('ContactCategory', ['VERY_LOW', 'LOW', 'MEDIUM', 'HIGH', 'VERY_HIGH'])->nullable();
            $table->enum('HasMobilePhone', ['true', 'false'])->nullable();
            $table->enum('HasResidentialAddress', ['true', 'false'])->nullable();
            $table->enum('Province', ['WC', 'EC', 'NC', 'G', 'FS', 'M', 'L', 'NW', 'KN'])->nullable();
